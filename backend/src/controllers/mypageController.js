...
class MypageController {
  ...
  async getLikedYoutubeVideos(req, res) {
    ...
    try {
      console.log(`사용자 ID: ${user_id} - 좋아요 누른 유튜브 비디오 요청 수신`);
      const likedVideos = await userService.getLikedYoutubeVideos(user_id);
      console.log("좋아요를 누른 유튜브 비디오:", likedVideos);
      res.status(200).json(likedVideos);
    } catch (error) {
      console.error("좋아요를 누른 유튜브 비디오 정보를 가져오는 중 에러 발생:", error);
      res.status(500).json({ error: error.message });
    }
  }
