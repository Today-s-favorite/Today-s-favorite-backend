...
class UserController {
  ...
  async getLikedYoutubeVideos(req, res) {
   const { user_id } = req.params;

   try {
      console.log(`사용자 ID: ${user_id} - 좋아요 누른 유튜브 동영상 요청 수신`);
      const likedYoutubeVideos = await userService.getLikedYoutubeVideos(user_id);
      console.log("좋아요를 누른 유튜브 동영상:", likedYoutubeVideos);
      res.status(200).json(likedYoutubeVideos);
    } catch (error) {
      console.error("좋아요를 누른 유튜브 동영상 정보를 가져오는 중 에러 발생:", error);
      res.status(500).json({ error: error.message });
    }
}
